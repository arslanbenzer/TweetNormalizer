...
public class RunTagger {
	...
	public void runTagger() throws IOException, ClassNotFoundException {
		...
		LineNumberReader reader = new LineNumberReader(BasicFileIO.openFileToReadUTF8("libraries/tweets.txt"));
		String line;
...
//			if (showConfidence) throw new RuntimeException("--confidence only works with greedy decoder right now, sorry, yes this is a lame limitation");
	...
	public String[][] runTaggerTweet(String tweet) throws IOException, ClassNotFoundException {		
		tagger = new Tagger();
		if (!justTokenize) {
			tagger.loadModel(modelFilename);			
		}
		...
		sentence.tokens = Twokenize.tokenizeRawTweetText(tweet);
		ModelSentence modelSentence = null;
		...
		for (int t=0; t < T; t++) {
			tokens[t] = sentence.tokens.get(t);
			tags[t] = tagger.model.labelVocab.name(modelSentence.labels[t]);
			confs[t] = formatConfidence(modelSentence.confidences[t]);
		}
		String [][] result =new String [3][T];
		result[0]=tokens;
		result[1]=tags;
		result[2]=confs;
		numtoks += sentence.T();
...
/*		int i=0;
	...
	public String[][] runTweet(String[] args) throws IOException, ClassNotFoundException {        
		...
		return tagger.runTaggerTweet(tweet);	//returns array of token tags and confidences 	
	}
